...
import moviecdramaRepositoryStore, { IMG_URL } from "../../store/apitopcdrama";
...
export default {
  ...
  computed: {
    ...mapState(moviecdramaRepositoryStore, ["crepository"]),
  },
  mounted() {
    this.fetchcRepository();
    this.$nextTick(() => {
  ...
  methods: {
    ...mapActions(moviecdramaRepositoryStore, ["fetchcRepository"]),
    scrollLeft() {
      const container = this.$refs.movieContainer;
